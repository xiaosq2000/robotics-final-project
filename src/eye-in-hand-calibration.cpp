...
//R和T转RT矩阵
const string data_path = "C:\\toy-projects\\robotics-final-project\\robotics-final-project\\data\\eye-in-hand-calibration"; //修改为自己的文件路径
//矩阵拆分
...
void eye_in_hand()
{
	...
	ifstream filein(data_path + "calibration.txt"); //读取图片名
	if (!filein)
	{
		cout << "未找到文件！！！" << endl;
	}
	ofstream fout(data_path + "Result.txt");
	cout << "角点获取" << endl;
	...
	while (getline(filein, imagename))
	{
		...
		imagin = imread(data_path + imagename); //读入图片
		if (imagcount == 1)
		{
		...
		cvtColor(imagin, grayImag, COLOR_RGB2GRAY);					//将图片转为灰度图
	...
	cout << "标定结束！！" << endl;
	cout << "开始保存结果...." << endl;
	ifstream ifile(data_path + "rpy.txt", ios::in);
	if (!ifile)
	{
