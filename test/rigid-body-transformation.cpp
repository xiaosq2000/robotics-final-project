...
/**
 * @brief The entry of the test of `rigid-body-transformation` application.
 * 
 * 
 * @return int 
 */
 ...
 * 
 */
RigidBodyTransformation::RigidBodyTransformation(const std::string &path, const double &l1, const double &l2, const double &l3, const double &l4)
{
    cv::Mat cam_int_param, cam_ext_param;
    cv::FileStorage fs1(path + "/camera_intrinsic_matrix.yml", cv::FileStorage::READ);
    fs1["camera_intrinsic_matrix"] >> cam_int_param;
    fs1.release();
    cv::cv2eigen(cam_int_param, this->cam_int_param_);

    cv::FileStorage fs2(path + "/camera_extrinsic_matrix.yml", cv::FileStorage::READ);
 ...
 * 
 */
void RigidBodyTransformation::InverseKinematics(const double *g_vec, double *joint_coor, bool *config)
{
    Vector3d v[6];
    for (int i = 0; i < 6; i++)
        v[i] = -w[i].cross(q_[i]);
 ...
 * 
 */
void RigidBodyTransformation::World2Joint(const double *rpy_coor, double *joint_coor)
{
 ...
 * 
 */
void RigidBodyTransformation::ForwardKinematics(const double *joint_coor, double *g_vec, bool *config)
{
    Vector3d v[6];
    for (int i = 0; i < 6; i++)
        v[i] = -w[i].cross(q_[i]);
 ...
 * 
 */
void RigidBodyTransformation::Joint2World(const double *joint_coor, double *rpy_coor)
{
 ...
 * 
 */
void RigidBodyTransformation::Pixel2Camera(const double *pixel_coor, double *camera_coor)
{
    double z = 380;
 ...
 * 
 */
void RigidBodyTransformation::Camera2Tool(const double *camera_coor, double *tool_coor)
{
